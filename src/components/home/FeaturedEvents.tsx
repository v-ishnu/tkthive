...
export const FeaturedEvents: React.FC<FeaturedEventsProps> = ({ events }) => {
  ...
  const getStatus = (event: EventData) => {
    ...
    if (event.venue.toLowerCase().includes('online')) return 'ONLINE';
    ...
    <div className="container mx-auto w-full">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="hidden md:flex gap-2">
            <button className="w-10 h-10 rounded-full border border-white/10 hover:bg-white/5 hover:border-primary/50 text-white flex items-center justify-center transition-all">
      ...
      <div className="flex flex-col md:flex-row gap-4 h-auto min-h-[800px] md:min-h-0 md:h-[450px]">
                ...
                <div className="absolute inset-0 p-4 md:p-6 flex flex-col justify-between">
                    ...
                    `}>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <p className="text-primary font-bold text-xs md:text-sm tracking-widest uppercase">{event.category} • {event.venue.toLowerCase().includes('online') ? 'Virtual' : 'Live'}</p>
                        </div>
